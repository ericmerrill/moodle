...
/**
 * The library file for the memcachecluster cache store.
 *
 * This file is part of the memcachecluster cache store, it contains the API for interacting with an instance of the store.
 *
...
/**
 * The memcache cluster store class.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cachestore_memcachecluster extends cachestore_memcache {
    /**
    ...
    public function __construct($name, array $configuration = array()) {
        ...
        // Now reference $this->connection to $this->getconnection.
        ...
        foreach ($this->setservers as $setserver) {
            ...
            $connection->addServer($setserver[0], $setserver[1], true);
            // Test the connection to the server.
            $ready = $ready && (@$connection->set($this->parse_key('ping'), 'ping', MEMCACHE_COMPRESSED, 1));
    ...
    public function instance_deleted() {
        if ($this->getconnection) {
            ...
            foreach ($this->servers as $server) {
                $connection->addServer($server[0], $server[1], true);
            }
        }
        @$connection->flush();
        unset($connection);
     ...
     * @return cachestore_memcachecluster|false
     */
    public static function initialise_test_instance(cache_definition $definition) {
        ...
        $config = get_config('cachestore_memcachecluster');
        ...
        $store = new cachestore_memcachecluster('Test memcache cluster'.$config->testserversget, $configuration);
