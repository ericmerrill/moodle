...
function xmldb_block_myoverview_upgrade($oldversion) {
    ...
    if ($oldversion < 2019091801) {
        ...
        if ($dbval !== false) {
            set_config('displaygroupingfavourited', $dbval, 'block_myoverview');
            unset_config('displaygroupingstarred', 'block_myoverview');
        }

        if (isset($CFG->forced_plugin_settings['block_myoverview']['displaygroupingfavourites'])) {
            ...
            $warn = 'Setting block_myoverview->displaygroupingstarred has been renamed '.
                    'to block_myoverview->displaygroupingfavourites';
            echo $OUTPUT->notification($warn, 'notifyproblem');
        }

        upgrade_block_savepoint(true, 2019091801, 'myoverview', false);
    }
