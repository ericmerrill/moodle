...
EditorClean.prototype = {
    ...
    pasteCleanup: function(source) {
        ...
        if (source.type === 'paste') {
            ...
            // Check if we have a valid clipboardData object in the event. Old browsers may not.
    ...
    fallbackPasteCleanup: function() {
        Y.log('Used fallbackPasteCleanup for atto cleanup', 'debug', LOGNAME);

        // Save the current selection (cursor position).
        var selection = window.rangy.saveSelection();

        // Get, clean, and place the content in the editable.
    ...
    /**
     * Calls fallbackPasteCleanup on a short timer to allow paste event handlers to complete.
     *
