...
EditorClean.prototype = {
    ...
    _cleanHTML: function(content) {
        ...
        return this._cleanHTMLLists(this._filterContentWithRules(content, rules));
    },
     ...
     * - An extra closing li tag will be discarded.
    ...
    _cleanHTMLLists: function(content) {
        var output = '',
            ...
            close = null,
            fulltag = null,
            levels = {ul: 0, ol: 0, li: 0};

        this._countTags(content);

        Y.log('Start', 'debug', LOGNAME);
        //while ((position = working.search(regex)) !== -1) {
            ...
            if (tag == 'ol' || tag == 'ul') {
                if (close) {
                    if (levels[tag] < 1) {
                        ...
                        // TODO Something.
                        Y.log('Closing a ' + tag + ' tag that isn\'t open.', 'debug', LOGNAME);
                    }
                    levels[tag]--;
                } else {
                    // Open OL tag.
