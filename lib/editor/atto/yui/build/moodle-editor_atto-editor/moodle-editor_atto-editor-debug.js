...
EditorClean.prototype = {
    ...
    _cleanHTMLLists: function(content) {
        ...
        while ((match = working.match(regex))) {
            ...
            if (openTags.length > 0) {
                for (var i = openTags.length - 1; i >= 0; i--) {
                    if (openTags[i].tag == 'ul' || openTags[i].tag == 'ol') {
                        currentList = openTags[i];
                    }
                }
            }
            ...
            if (close) {
                if (openTags.length == 0) {
                    ...
                    // Just discarding the extra close tag should be safe.
                    Y.log('Discarding extra ' + tag + ' close tag.', 'debug', LOGNAME);
                    fullTag = '';
                } else {

                    if (openTags[openTags.length - 1].tag != tag) {
                        // This means we are closing a tag that isn't the most recent open one open.
                        if (tag == 'li' && currentList) {
                            // This will look for the most recent (last) li close, ol open, or ul open.
                            if ((match = output.match(/(<(?:\/li|ol|ul)[^>]*>)(?![\s\S]*<(\/li|ol|ul)[ >])/i))) {
                                // If we found it, insert an open li tag there to match this one.
                                Y.log('Inserting opening li tag.', 'debug', LOGNAME);
                                output = this._insertString(output, '<li>', match.index + match[0].length);
                                currentList.li++;
                            } else {
                                Y.log('Discarding incorrect ' + tag + ' close tag.', 'debug', LOGNAME);
                                fullTag = '';
                            }
                        } else {
                            if (openTags[openTags.length - 1].tag == 'li') {
                                // If we are in this case, then we can assume we are just missing a li closing tag.
                                output += '</li>';
                                openTags.pop();
                            } else {
                                Y.log('Discarding incorrect ' + tag + ' close tag.', 'debug', LOGNAME);
                    ...
                    if (tag == 'li' && fullTag.length && currentList) {
                        currentList.liEnd = output.length + fullTag.length;
                    }
                }
            } else {

                // We have to do special processing for list items.
                if (tag == 'li') {
                    if (currentList) {
                        currentList.li++;
                    } else {
                        ...
                        if (counts.ol.close > counts.ol.open) {
                            // There is an extra ol close tag up ahead. Assume there was a missing ol tag and insert one.
                            Y.log('Added an opening ol tag', 'debug', LOGNAME);
                            openTags.push({tag: 'ol', position: output.length, length: 4, li: 1});
                            output += '<ol>';

                        } else if (counts.ul.close > counts.ul.open) {
                            // There is an extra ul close tag up ahead. Assume there was a missing ul tag and insert one.
                            Y.log('Added an opening ul tag', 'debug', LOGNAME);
                            openTags.push({tag: 'ul', position: output.length, length: 4, li: 1});
                            output += '<ul>';

                        } else {
                            // There are no unbalanced ul/ol tags, so we want to find the limits of these li tags, and wrap them.
                            // First confirm there is an extra li closing tag ahead. There should be since we are mid tag.
                ...
                // If we haven't cleared the tag above for some reason, record it to the open tags list.
                if (fullTag.length > 0) {
                    openTags.push({tag: tag, position: output.length, length: fullTag.length, li: 0});
                }
            }

            Y.log(JSON.stringify(openTags), 'debug', LOGNAME);
            output += fullTag;
        }
        ...
        if (openTags.length) {
            ...
            while ((currentList = openTags.pop())) {
                if (currentList.li > 0) {
                    if (currentList.liEnd) {
                        // We have a position for the last list item in this element. Close it after that.
                        output = output.slice(0, currentList.liEnd) +
                                '</' + currentList.tag + '>' +
                                output.slice(currentList.liEnd);
                        Y.log('Added closing ' + currentList.tag + ' based on last li location.', 'debug', LOGNAME);
                    } else {
                        // No idea where it goes, so just end it here.
                        output += '</' + currentList.tag + '>';
                        Y.log('Added closing ' + currentList.tag + ' to end of content.', 'debug', LOGNAME);
                    }
                } else {
                    // If there weren't any children list items, then we should just remove the tag where it started.
                    output = output.slice(0, currentList.position) + output.slice(currentList.position + currentList.length);
                    Y.log('Removing ' + currentList.tag + ' opening tag because it was missing closing.', 'debug', LOGNAME);
                }
            }
        }
