...
//         this._cleanHTMLLists2(content);
        content = this._cleanHTMLLists(content);

        return content;
        //return this._cleanHTMLLists(content);
    },
    ...
    _cleanHTMLLists2: function(content) {
        Y.log(content, 'debug', LOGNAME);

        var processed = this._cleanHTMLRecurse(null, null, content);

        Y.log(processed.content, 'debug', LOGNAME);
    },

    _cleanHTMLRecurse: function(tag, parenttag, content) {
        var output = {content: '', remainder: ''},
            working = content,
            regex = /<(\/?)(li|ul|ol)[^>]*>/i,
            position = null,
            match = null,
...
//
//         while ((match = working.match(regex))) {
//             position = match.index;
//             localtag = match[2].toLowerCase(); // Standardize to lower case.
//             close = (match[1].length == 1) ? true : false;
//             fulltag = match[0];
//
//
//
//         }
        ...
        while ((match = working.match(regex))) {
            ...
            output.content += working.slice(0, position);
            working = working.slice(position + fulltag.length);

            if (!close) {
                lastopen = localtag;
                var results = this._cleanHTMLRecurse(localtag, tag, working);
                working = results.remainder;
                output.content += fulltag + results.content;
            } else {
                if (localtag != tag) {
                    Y.log("Invalid closing! " + localtag + " " + tag, 'debug', LOGNAME);
                }

                output.content += fulltag;
                output.remainder = working;
     ...
     * - An extra closing li, ul, or ol tag will be discarded.
     * - If there is an unmatched list open tag, a matching close tag will be inserted.+
     * ---- If there is an unmatched closing ul or ol tag, a matching open tag will be inserted.+
     *
    ...
    _cleanHTMLLists: function(content) {
        var output = '',
            ...
            close = null,
            fulltag = null,
            levels = {ul: [], ol: [], li: []}; // Tracks the output position of any open tags.

        Y.log('Start', 'debug', LOGNAME);

        while ((match = working.match(regex))) {
            position = match.index;
            tag = match[2].toLowerCase(); // Standardize to lower case.
            close = (match[1].length == 1) ? true : false;
            fulltag = match[0];

            // Slice up the content based on the match.
            // Add content before the match to output.
            // The match is in fulltag, and working will now contain everything after the match
            output += working.slice(0, position);
            working = working.slice(position + fulltag.length);

            if (tag == 'ol' || tag == 'ul') {
                if (close) {
                    if (levels[tag].length < 1) {
                        // This means we are closing a tag that isn't open. That's a problem.
                        // TODO Something.
                        Y.log('Discarding extra ' + tag + ' close tag.', 'debug', LOGNAME);
                        fulltag = '';
                    } else {
                        levels[tag].pop();
                    }
                    ...
                    // Open OL tag.
                    levels[tag].push(output.length);
                }
            } else if (tag == 'li') {
                if (close) {
                    if (levels.li.length < 1) {
                        // This would mean we are closing a li tag that isn't open.
                        // For li, it should be safe to just discard this closing.
                        Y.log('Discarding extra li close tag.', 'debug', LOGNAME);
                        fulltag = '';
                    } else {
                        levels.li.pop();
                    }
                } else {
                    levels.li.push(output.length);
                    if (levels.ul.length == 0 && levels.ol.length == 0) {
                        // This means we have a list item not inside a list. This is a problem!
                        // See how many ul and ol tags are left.
                        var counts = this._countTags(working);

                        if (counts.ol.close > counts.ol.open) {
                            // There is an extra ol close tag up ahead, but we aren't open. So we can open one.
                            Y.log('Added an opening ol tag', 'debug', LOGNAME);
                            levels.ol.push(output.length);
                            output += '<ol>';

                        } else if (counts.ul.close > counts.ul.open) {
                            // There is an extra ul close tag up ahead, but we aren't open. So we can open one.
                            Y.log('Added an opening ul tag', 'debug', LOGNAME);
                            levels.ul.push(output.length);
                            output += '<ul>';

                        } else {
                            // There are no unbalanced tags ul/ol, so we want to find the limits of these li tags, and wrap them.
                            // First confirm there is an extra closing tag ahead.
                            if (counts.li.close > counts.li.open) {
                                // Open a new ul tag.
                                Y.log('Wrapping orphan li tags in a ul set.', 'debug', LOGNAME);
                                output += '<ul>';
                                levels.ul.push(output.length);

                                // We want to find the last li close tag before any other lists, and insert the new close tag there.
                                var searchSpace = working;
                                if ((position = searchSpace.search(/<(ol|ul)[ >]/i)) > -1) {
                                    searchSpace = searchSpace.slice(0, position);
                                }

                                // Now we want to see where the last close li is, and insert the close ul after that.
                                if ((position = searchSpace.search(/<\/li[ >](?![\s\S]*<\/li[ >])/gi)) == -1) {
                                    // This should never happen. If it does, just put it at the end of our search space...
                                    position = searchSpace.length;
                                }

                                // Insert the closing ul tag up ahead.
                                working = this._insertString(working, '</ul>', position);

                            } else {
                                // This would mean that there is no closing li tag up ahead, so just ditch this li tag.
                                fulltag = '';
                                levels.li.pop();
                            }
...
//             Y.log(JSON.stringify(levels), 'debug', LOGNAME);
//             Y.log(levels.ul, 'debug', LOGNAME);
            output += fulltag;
        }

        // TODO - check for extra tags.

        output += working;
        Y.log('Done', 'debug', LOGNAME);
        return output;
    },

    /**
     * Counts the number of opening and closing li, ul, and ol tags in a given bit of string.
     *
     * @method _countTags
     * @private
     * @param {String} content The string to count the tags in.
     * @return {Object} A nested object with the count of open and close of each tag type.
     */
    _countTags: function(content) {
        var regex = /<(\/?)(li|ul|ol)[^>]*>/ig,
            output = {ul: {open: 0, close: 0},
                      ol: {open: 0, close: 0},
                      li: {open: 0, close: 0}},
            match = null;
        ...
        while ((match = regex.exec(content)) !== null) {
            output[match[2].toLowerCase()][(match[1].length == 1) ? 'close' : 'open']++;
        }

        Y.log(JSON.stringify(output), 'debug', LOGNAME);

        return output;
    },

//     _cleanLists: function(content) {
//
//         var licount = (content.match(/<li[ >]/gi) || []).length,
//             ulopencount = (content.match(/<ul[ >]/gi) || []).length,
//             ulclosecount = (content.match(/<\/ul[ >]/gi) || []).length,
//             olopencount = (content.match(/<ol[ >]/gi) || []).length,
//             olclosecount = (content.match(/<\/ol[ >]/gi) || []).length;
//
//         if (ulopencount > ulclosecount) {
//             // Missing ul close tags.
//
//         }
//
//         if (ulopencount < ulclosecount) {
//             // Missing ul open tags. Insert new tag before first li, or start of string.
//             var position = content.search(/<li[ >]/gi);
//             if (position === -1) {
//                 position = 0;
//             }
//
//             content = this._insertString(content, '<ul>'.repeat(ulclosecount - ulopencount), position);
//
//             ulopencount += count;
//         }
//
//         if (olopencount > olclosecount) {
//             // Missing ol close tags.
//
//         }
//
//         if (olopencount < olclosecount) {
//             // Missing ol open tags. Insert new tag before first li, or start of string.
//             var position = content.search(/<li[ >]/gi);
//             if (position === -1) {
//                 position = 0;
//             }
//
//             content = this._insertString(content, '<ol>'.repeat(ulclosecount - ulopencount), position);
//
//             ulopencount += count;
//
//         }
