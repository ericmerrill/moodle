...
class engine extends \core_search\engine {
    ...
    protected function process_document_files($document) {
        ...
        foreach ($files as $file) {
            // Check if we should index the file, and do it if so.
    ...
    protected function add_stored_file($document, $storedfile) {
        ...
        try {
            ...
            $result = $curl->post($strurl, array('myfile' => $storedfile));
            ...
            if ($code != 0) {
                ...
                $message = 'HTTP error code '.$info['http_code'].' while indexing file with document id '.$filedoc['id'];
                // Try to get error message out of title if it exists.
